...
`A` is the type of the result. Basic purpose of the `State` data structure is to provide mechanisms for composing 
...
such implementation takes 1.5 s per frame. Following paragraphs explain how those 1.5 s have been lowered to around 
45 ms per frame.
...
needed to update them on every clock tick. The goal in this case was to remove `scanline` and `cycle` counters 
from the state case classes. A possible solution was to create the frame processing function in advance, or in other 
words, to replace a loop with a function composition.
...
increments with each clock tick, `scanline` defines a row, while `cycle` defines a column within PPU timing diagram. 
Method `clock` returns a `State` object (or state transformation function) for specified clock tick. If no 
transformation is needed then `clock` method returns `None`. All those transformations are combined with `*>` method of 
the `State` data structure (a variant of the `flatMap` method). As a result, `executeFrame` contains a state 
transformation for the complete frame. The frame processing function doesn't need counters as part of the `NesState` 
case class any more, which means faster frame processing. On the other hand, having a fixed transformation showed to be 
...
created and old one is destroyed. In order to minimize the number of state transformations, PPU in this 
emulator implementation outputs 8 pixels at a time. 
